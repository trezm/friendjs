...
module.exports.UserController = {
	create: function(params, callback) {
		if (config.ADMIN_ACCOUNTS_ENABLED) {
			delete params.isAdmin;
		}

		if (config.ACCOUNT_APPROVAL_ENABLED) {
		...
		User.create(params, function(error, results) {
			if (error) {
				...
				if (!!(config.MANDRILL_API_KEY != undefined &&
	...
	create: function(req, res) {
		...
		module.exports.UserController.create(user, function(error, results) {
			...
			if (config.MANDRILL_API_KEY &&
				config.MANDRILL_CONFIRMATION_ADDRESS && 
				user.email) {
			    ...
			    config.MANDRILL_FROM,
			    config.MANDRILL_SUBJECT
			    );
			}
		});
	},

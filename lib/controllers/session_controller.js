...
module.exports.SessionController = function(options) {
	return {
		create: function(params, callback) {
			...
			async.waterfall([
				function confirmPin(callback) {
					if (!params.email) {
						options.redis.get('pin.confirmation.' + query.phoneNumber, function(error, results) {
				...
				function createDevice(user, callback) {
					if (params.device && params.device.deviceId) {
						DeviceController(options).create(user, params.device.deviceId, function(error, results) {
							callback(error, user);
						});
					} else {
						callback(null, user);
	    ...
	    createPin: function(redis, twilio, params, callback) {
	    	...
	    	async.series({
	    		...
	    		setRedis: function(next) {
	    			options.redis.set('pin.confirmation.' + libphonenumber.e164(params.toNumber, 'US'), pin, 'EX', 60, next);
	    		}
	    	}, callback);
	    },
...
module.exports.SessionControllerRoutes = function(options) {
	var SessionController = module.exports.SessionController(options);
	...
	create: function(req, res) {
		if (req.body.email && req.body.password) {
			...
			SessionController.create(session, function(error, results) {
				if (error) {
					res.status(403).json({
						error: error
					});
	...
	requestPin: function(req, res, next) {
		...
		if (validate({
			...
			SessionController.createPin(options.redis, options.twilio, {
				toNumber: req.params.phoneNumber,
				fromNumber: options.twilioNumber
			}, function(error, results) {
				if (error) {
					res.status(500).json(error);
				} else {

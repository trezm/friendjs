...
//						})
			...
			function(session, callback) { // Here we want to create or update the device token
				if (params.device) {
					if (Device.DeviceTypes.indexOf(params.device.deviceType) < 0) {
						params.device.deviceType = 'unknown'
					}
					Device.create({
						type: params.device.deviceType,
